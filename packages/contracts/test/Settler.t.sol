...
contract SettlerTest is Test {
  ...
  MockPriceOracle public TCAP_PRICE_ORACLE;
  ...
  function test_ShouldDeployWithCorrectParameters() public view {
    ...
    assertEq(address(settler.TCAP_PRICE_ORACLE()), address(TCAP_PRICE_ORACLE));
  ...
  function test_OrderShouldFailIfTheInstitutionIsNotAllowed() public {
    ...
    // 1 CRYPTO40 = 250 USDC
    settler.createOrder(1 ether, address(USDC), 250 ether);
    vm.stopPrank();
  }
  ...
  function test_AllowedInstitutionsCanCreateOrders() public {
    ...
    settler.createOrder(1 ether, address(USDC), 250_000_000);
    vm.stopPrank();
  ...
  // Test: Only market makers can fill orders
  function test_OnlyMarketMakersCanFillOrders() public {
  ...
  // Test: Order cannot be filled if is expired
  function test_OrderCannotBeFilledIfIsExpired() public {
    MockERC20(address(USDC)).mint(INSTITUTION, 250_000_000); // 250 USDC
    vm.startPrank(ADMIN);
    settler.setInstitutionStatus(INSTITUTION, true);
    settler.setMarketMakerStatus(MARKET_MAKER, true);
  ...
  function test_MarketMakersCanFillAnOrder() public {
    MockERC20(address(USDC)).mint(INSTITUTION, 250_000_000); // 250 USDC
    vm.startPrank(ADMIN);
    settler.setInstitutionStatus(INSTITUTION, true);
    settler.setMarketMakerStatus(MARKET_MAKER, true);
  ...
  // Test: Should fail if order is filling
  function test_ShouldFailIfOrderIsFilling() public {
    MockERC20(address(USDC)).mint(INSTITUTION, 250_000_000); // 250 USDC
    vm.startPrank(ADMIN);
    settler.setInstitutionStatus(INSTITUTION, true);
    settler.setMarketMakerStatus(MARKET_MAKER, true);
  ...
  function test_AllowedMarketMakersCanCompleteAnOrder() public {
    ...
    // tcap.createOrder(1 ether, address(USDC), 250_000_000);
    // vm.stopPrank();
    // vm.startPrank(MARKET_MAKER);
    // tcap.fillOrder(1);
    // vm.stopPrank();
  ...
  // Test: Burning TCAP creates an intent
  function test_BurningTCAPCreatesIntent() public {
    // TODO: Implement test
    revert("Not implemented");
  }
