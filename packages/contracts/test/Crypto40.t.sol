...
contract Crypto40Test is Test {
  ...
  function test_onlyOwnerCanSetTransferable() public {
    vm.expectRevert(abi.encodeWithSelector(Ownable.OwnableUnauthorizedAccount.selector, address(this)));
    crypto40.setTransferable(true);
    vm.stopPrank();
  }

  function test_ownerCanEnableTransfer() public {
    vm.startPrank(OWNER);
    crypto40.mint(OWNER, 1000);
    crypto40.setTransferable(true);
    ...
    assertEq(crypto40.balanceOf(OWNER), 0);
    vm.prank(USER);
